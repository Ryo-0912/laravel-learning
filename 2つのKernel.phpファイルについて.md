Laravelには、```app/Http/Kernel.php ```と```app/Console/Kernel.php ```の2つのファイルが存在する。
それぞれ役割が異なる。

- 1 **app/Http/Kernel.php**
  HTTPリクエストの処理をどのようにするかを定義しているファイル。
  middlewareの設定を主に行なっている。
...
class Kernel extends HttpKernel
{
    ...
    protected $middlewareAliases = [
        ...
        'jwt.auth.custom' => \App\Http\Middleware\VerifyJwtToken::class,
    ];
...
このファイルで定義を行うと、```Route::middleware('verified')->```などとして使うことができる。


- 2 **app/Console/Kernel.php**
  バッチなどのコンソールで実行するコマンドの定義を行なっているファイル。
...
ここに、定義されているコマンドは```php artisan batch:user-induction-email```などとしてコンソールでコマンド実行できる。
